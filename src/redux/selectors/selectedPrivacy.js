    ...
    memoize((tokenID) => {
      ...
      try {
        ...
        for(let i = 0; i < listUnifiedToken?.length; i++) {
          const tokenId = listUnifiedToken[i]?.tokenId;
          const tokenData = listUnifiedToken[i];
          const unifiedTokenData = new SelectedPrivacy(
            account,
            null,
            tokenData,
            tokenId,
          );
          unifiedTokens.push(unifiedTokenData);
        } 
