...
import {defaultAccountSelector, listAccountSelector} from '@src/redux/selectors/account';
...
const Item = React.memo(({ account, handleDelete, handleSwitchAccount, handleExportKey, isLast }) => {
  ...
  return(
    ...
    >
      <TouchableOpacity style={itemStyled.wrapContent} onPress={() => handleExportKey(account)}>
        <Row centerVertical spaceBetween>
          <Row centerVertical>
            <TouchableOpacity style={{ paddingRight: 16 }} onPress={() => handleSwitchAccount(account)}>
        ...
        <Text style={itemStyled.mediumGrey} ellipsizeMode="middle" numberOfLines={1}>{account.paymentAddress}</Text>
      </TouchableOpacity>
    </Swipeout>
  );
});
