...
const Input = React.memo(({ onWithdrawMaxInvest }) => {
  ...
  const { withdrawInvest, token } = useSelector(
    stakingSelector.withdrawInvestCoinSelector,
  );

  const onChangeText = (text) =>
    dispatch(
      change(
        formConfigsWithdrawInvest.formName,
        formConfigsWithdrawInvest.input,
        text,
      ),
    );
  const onChangeWithdrawMax = () =>
    withdrawInvest && onWithdrawMaxInvest(withdrawInvest.maxWithdrawAmountStr);
  return (
    <>
      <Field
